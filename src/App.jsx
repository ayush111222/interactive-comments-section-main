...
function App() {
  ...
  const updateScore = (updatedScoreComment)=>{
    const newComment = [...comments]
    const index = comments.findIndex(comment=>comment.id === updatedScoreComment.id)
  ...
  const deletePopUpWithReplyId = (childCommentId, parentCommentId) => {
    ...
    setDeleteCommentAndReplyId({parentCommentId: parentCommentId, childCommentId: childCommentId})
  };

  const deleteReply = ()=>{
    const newComments = [...comments];
    if(deleteCommentAndReplyId.childCommentId === null){
      const index = comments.findIndex(comment => comment.id === deleteCommentAndReplyId.parentCommentId)
      ...
      const deletedReply = {
        ...findComment, replies : filteredReplyArray
      }
      const index = comments.findIndex(comment => comment.id === deleteCommentAndReplyId.parentCommentId)
  ...
  const updateParentAndChildContent = (updatedParentCommentContent, updatedChildCommentContent)=>{
    const newComments = [...comments];
    if(updatedChildCommentContent){
      ...
      const indexOfComment = comments.findIndex(comment => comment.id === updatedParentCommentContent)
      console.log(indexOfComment);
      const indexOfCommentReply = findComment.replies.findIndex(reply => reply.id === updatedChildCommentContent.id)
    ...
    const index = comments.findIndex(comment => comment.id === updatedParentCommentContent.id)
